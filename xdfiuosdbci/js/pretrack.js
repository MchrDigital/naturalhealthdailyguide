var campaignID = "6a42df036433d1a30771aa2a";
...
var initialSrc = "https://rt.livemybestday.com/"+campaignID+"?format=json";
...
setTimeout(function(){
	if (!urlParams.get('rtkcid')) {
		...
		xhr.onreadystatechange = function() {
			if (xhr.readyState == 4 && xhr.status == 200) {
				...
				document.querySelectorAll('a').forEach(function(el) {
					if (el.href.indexOf("rt.livemybestday.com/preclick")>-1) {
				...
				xhrr.open("GET", "https://rt.livemybestday.com/preview?clickid="+rawData.clickid)
				xhrr.send();
			}
		}
		...
		xhrTrack.open("GET", "https://rt.livemybestday.com/preview?clickid="+rtkClickID )
		xhrTrack.send();
		document.querySelectorAll('a').forEach(function(el) {
			if (el.href.indexOf("rt.livemybestday.com/preclick")>-1) {
				if (el.href.indexOf('?')>-1) {
